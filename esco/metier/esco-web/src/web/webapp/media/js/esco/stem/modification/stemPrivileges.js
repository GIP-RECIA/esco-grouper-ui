 ...
 * @author aChesneau
 */
var StemPrivilegesPrototype = $.extend( true, {}, EscoGrid ,{
    ...
    */
	getPostDataToFindDataRequest : function(){
		var json = $.extend({
			stemUuid : $("input[id=stemUuid]").val(),
    ...
    */
	...
	doOnLoadComplete:function(data){
		...
		for ( var i = 0; i < ids.length; i++) {
			...
			if( !(privilegeRadioValue == "PRIVILEGE_RADIO_EFFECTIVE" || privilegeRadioValue == "PRIVILEGE_RADIO_ALL")) {
				$("#" + cl + "_col3").bind("click", ActionRightCheckBox.clickHasStemDefaultAction(this._options,stem.getIsStemModified));
				$("#" + cl + "_col3").attr("action" , "stem");

				$("#" + cl + "_col4").bind("click", ActionRightCheckBox.clickHasCreateDefaultAction(this._options,stem.getIsStemModified));
				$("#" + cl + "_col4").attr("action" , "group");
	...
	doAddNavButtons:function(){
		...
		var addAction = ActionNavBar.actionAddStemPrivilege(_this,"/" + Core.applicationContext + "/stylesheets/stemModifications.jsf",1);
		this._addANavButton(
		...
		var delAction = ActionNavBar.actionDelDefault(_this,"/" + Core.applicationContext + "/ajax/stemModificationsPrivilegesController/deleteItems.jsf",stem.getIsStemModified);
		this._addANavButton(
