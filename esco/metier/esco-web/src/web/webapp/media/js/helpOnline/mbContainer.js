...
(function($){
  ...
  $.doOnWindowResize=function(el){
    ...
    this.doRes=setTimeout(function(){
      $(el).adjastPos();
      winw=$("body").width();	winh=$("body").height();
    },400);
  };
  ...
  jQuery.fn.buildContainers = function (options){
    ...
    return this.each (function (){
      ...
      if (this.options.mantainOnWindow)
        $("body").resize(function(){$.doOnWindowResize(el);});
      var container=$(this);
  ...
  jQuery.fn.containerResize = function (){
    ...
    container.resizable({
      ...
      helper: "mbproxy",
      start:function(e,o){
        $(container).resizable('option', 'maxHeight',$("body").height()-($(container).offset().top)-5);
        $(container).resizable('option', 'maxWidth',$("body").width()-$(container).offset().left-5);
  ...
  jQuery.fn.mb_open = function (url,data){
    ...
    if (container.mb_getState('closed')){
      ...
      if(!container.mb_getState("collapsed")){
        container.mb_resizeTo(350,500,false);
      }
  ...
  jQuery.fn.mb_fullscreen= function(){
    ...
    if (container.mb_getState('iconized') || container.mb_getState('collapsed') || container.mb_getState('closed')){
      container.attr("w",$("body").width()-40);
      container.attr("h",$("body").height()-40);
      container.attr("t",20);
      container.attr("l",20);
    ...
    container.mb_resizeTo($("body").height()-40,$("body").width()-40);
