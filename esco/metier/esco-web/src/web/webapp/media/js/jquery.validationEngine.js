...
buildPrompt : function(caller,promptText,type,ajaxed) {			// ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
		...
		if ( $(caller).parent("td").html() !=null){

			//SOPRA

			// The input to validate is in an table
			if($("p."+$(caller).attr("id")+"formError").length!=0){
				$("p."+$(caller).attr("id")+"formError").remove();
			}
			...
			/*
				// The input to validate is in an table
				if($("div."+$(caller).attr("id")+"formError").length!=0){
					$("div."+$(caller).attr("id")+"formError").remove();
				}
				...
				$(caller).parent().prepend(divFormError);
				...
				callerTopPosition = 50;//$(caller).offset().top - $("#mainContent").offset().top;
				callerleftPosition = 50;//$(caller).offset().left - $("#mainContent").offset().left;
				...
				paddingRight = ($(caller).parent().width() - $(caller).width() )/2;
				$(divFormError).css({
	...
	validateCall : function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
		...
		// function added by Sopra Group for the default case
		...
		/** function added by Sopra Group for the default case */
		function _customRegexDefault(caller,rules){
			pattern = eval(rules.regexp);
			if(!pattern.test($(caller).attr('value')) || $(caller).attr('value').indexOf("<") >= 0){
				$.validationEngine.isError = true;
				//promptText += $.validationEngine.settings.allrules[customRule].alertText+"<br />";
