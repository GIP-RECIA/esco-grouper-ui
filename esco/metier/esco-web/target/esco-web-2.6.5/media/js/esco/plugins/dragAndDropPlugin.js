...
}}}},moveElement:function(C,A,B,D){if($(C).attr("typenode")=="GROUP"){if(jQuery.isFunction(DragAndDrop.moveGroup)){return DragAndDrop.moveGroup(C,A,B,D)
}else{Core.log("Warning: The moveGroup is not a function");
return false
}}else{if($(C).attr("typenode")=="FOLDER"){if(jQuery.isFunction(DragAndDrop.moveStem)){return DragAndDrop.moveStem(C,A,B,D)
