...
var WelcomeBase={_options:{buttonBarHidden:false,buttonBarPresent:false,generalMenuAction:[]},outerLayout:null,init:function(A){_options=$.extend({},{rootName:"Root"},A||{})
...
this.initTree();
this.initLayer();
jQuery.each(this._options.generalMenuAction,function(A,B){B.call()
});
...
this.outerLayout=$(".flora").layout(C);
var B=".flora > .ui-layout-west";
$("<span></span>").attr("id","west-closer").prependTo(B);
...
},hide:function(B){try{this.outerLayout.hide(B)
}catch(A){}},show:function(B){try{this.outerLayout.show(B)
}catch(A){}},resizeAll:function(){this.outerLayout.resizeAll()
},fireButtonBar:function(){var A=this;
A._options.buttonBarPresent=$("#buttonBar").size()>0;
if(A._options.buttonBarPresent){isClosed=$("#buttonBarAction").hasClass("buttonBarClose");
A._options.buttonBarHidden=false||isClosed;
...
A._options.buttonBarHidden=false;
...
A._options.buttonBarHidden=true;
