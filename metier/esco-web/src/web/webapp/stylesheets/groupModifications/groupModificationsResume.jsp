...
<g:view stringsVar="msgs" locale="#{sessionController.locale}">
	...
	<input type="image" id="onlineHelpMain" name="ONLINE_HELP_GROUP_MODIFICATION_RESUME_URL" src="/<h:outputText value="#{sessionController.applicationContext}"/>/media/imgs/action/help.gif" title="<h:outputText value="#{msgs['ICON_HELP_TITLE']}" />" style="float:right;margin-right:20px;cursor:pointer;border:0;" />
		...
		<li id="resumeTab">
			<a href="#resumePanel">
				<span>
					...
					<h:outputText value="#{msgs['GROUP_MODIFICATIONS_RESUME.TITLE']}" />
				</span>
			</a>
		</li>
	</ul>
	<div id="escoPanels">
		<t:div id="resumePanel">
			<t:div id="errorPanel" styleClass="errorMessage" rendered="#{groupModificationsController.errors != null && not empty groupModificationsController.errors && empty resume.jsp}">
				<t:graphicImage url="/media/imgs/priv/11951.showerr_tsk.gif" />
				<h:outputText style="color:red" value="#{msgs['GROUP_MODIFICATIONS_RESUME.ERROR_DETAIL']}" />
				<t:dataList var="error" value="#{groupModificationsController.errors}">
					<f:verbatim>
					<ul>
						<li></f:verbatim>
							<h:outputText style="error" value="#{msgs[concat['org.esco.grouperui.groupmodification.resume. ,#error']]}" rendered="#{msgs[concat['org.esco.grouperui.groupmodification.resume. ,#error']] != concat['?????,org.esco.grouperui.groupmodification.resume. ,#error,?????']}" />
							<h:outputText style="error" value="#{msgs[error]}" rendered="#{msgs[concat['org.esco.grouperui.groupmodification.resume. ,#error']] == concat['?????,org.esco.grouperui.groupmodification.resume. ,#error,?????']}" />
						<f:verbatim></li>
					</ul>
					</f:verbatim>
				</t:dataList>
			</t:div>
			<t:dataList var="resume" value="#{groupModificationsController.listResume}">
				<t:div id="elementPanel" rendered="#{resume.data != null && not empty resume.data && resume.jsp == null}">
					<f:verbatim>
						<h2></f:verbatim>
							...
							<t:div id="dataDiv" style="width : 90%">
								<t:dataTable value="#{resume.data}"
										...
										<t:columns value="#{resume.colInfos}" var="colInfos" headerstyleClass="tableHeader" >
											<f:facet name="header">
												<t:outputText escape="false" value="#{msgs[colInfos.colName]}" />
											</f:facet>
												...
												<t:aliasBean  alias="#{typeDataValue}" value="#{resume.typeDataValue}">
													<t:aliasBean  alias="#{originalValue}" value="#{resume.originalValue}">
														...
														<t:div styleClass="editableText" id="editText" forceId="#{resume.rowNum}"  rendered="#{typeDataValue == 'ERROR' && colInfos.dataType == 'INPUT' && (empty resume.originalValue || resume.originalValue == null)}">
															<f:verbatim><strong class="flc-inlineEdit-text" style="color:red"></f:verbatim>
															<t:outputText value="#{columnValue}"/>
	...
	<script type="text/javascript">
		...
		var groupModificationsResume = new GroupModificationsResume();
		Core.addScreen(groupModificationsResume);
