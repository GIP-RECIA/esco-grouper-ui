...
<g:view stringsVar="msgs" locale="#{sessionController.locale}">
	...
	<input type="image" id="onlineHelpMain" name="ONLINE_HELP_GROUP_PROPERTIES_RESUME_URL" src="/<h:outputText value="#{sessionController.applicationContext}"/>/media/imgs/action/help.gif" title="<h:outputText value="#{msgs['ICON_HELP_TITLE']}" />" style="float:right;margin-right:20px;cursor:pointer;border:0;" />
		...
		<li id="resumeTab">
			<a href="#resumePanel">
				<span>
					...
					<h:outputText value="#{msgs['GROUP_RESUME.TITLE']}" />
				</span>
			</a>
		</li>
	</ul>
	<div id="escoPanels">
		<t:div id="resumePanel">
			<t:div id="errorPanel" styleClass="errorMessage" rendered="#{groupController.errors != null && not empty groupController.errors && empty resume.jsp}">
				<t:graphicImage url="/media/imgs/priv/11951.showerr_tsk.gif" />
				<h:outputText style="color:red" value="#{msgs['GROUP_RESUME.ERROR_DETAIL']}" />
				<t:dataList var="error" value="#{groupController.errors}">
					<f:verbatim>
					<ul>
						<li></f:verbatim>
							<h:outputText style="error" value="#{msgs[concat['org.esco.grouperui.group.resume. ,#error']]}" rendered="#{msgs[concat['org.esco.grouperui.group.resume. ,#error']] != concat['?????,org.esco.grouperui.group.resume. ,#error,?????']}" />
							<h:outputText style="error" value="#{msgs[error]}" rendered="#{msgs[concat['org.esco.grouperui.group.resume. ,#error']] == concat['?????,org.esco.grouperui.group.resume. ,#error,?????']}" />
						<f:verbatim></li>
					</ul>
					</f:verbatim>
				</t:dataList>
			</t:div>
			<t:dataList var="resume" value="#{groupController.listResume}">
				<t:div id="elementPanel" rendered="#{resume.data != null && not empty resume.data && empty resume.jsp}">
					<f:verbatim>
						<h2></f:verbatim>
							...
							<t:graphicImage rendered="#{resume.error && groupController.saveCall}" url="/media/imgs/priv/11951.showerr_tsk.gif" />
							<t:graphicImage rendered="#{not resume.error && not resume.saved && groupController.saveCall}" url="/media/imgs/priv/17370.contended_monitor_obj.png" />
							<t:graphicImage rendered="#{resume.saved && groupController.saveCall}" url="/media/imgs/priv/14073.complete_status.gif" />
							<h:outputText escape="false"  value="&#160;"/>
							<f:verbatim><h:outputText style="color:red" value="#{msgs['DEFAULT_ERROR_OPERATION']}" rendered="#{resume.error && groupController.saveCall}" /></f:verbatim>
						<f:verbatim></h2></f:verbatim>
					<t:div id="contentToOpenOrReduce" style="display:none;">
						<t:div styleClass="errorMessage">
							<f:verbatim><h:outputText style="color:red" value="#{msgs['DEFAULT_ERROR_ELEMENTS']}" rendered="#{resume.error && groupController.saveCall}" /></f:verbatim>
						</t:div>
	...
	<script type="text/javascript">
		...
		var groupResume = new GroupResume();
		Core.addScreen(groupResume);
