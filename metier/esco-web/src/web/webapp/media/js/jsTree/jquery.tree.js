...
(function($) {
	...
	function tree_component () {
		return {
			...
			reselect : function (is_callback) {
				...
				if(this.settings.selected !== false) {
					$.each(this.settings.selected, function (i) {
						if(_this.is_partial_refresh)	_this.select_node($(_this.settings.selected[i].toString().replace('/','\\/'), _this.container), (_this.settings.rules.multiple !== false) );
						else							_this.select_node($(_this.settings.selected[i].toString().replace('/','\\/'), _this.container), (_this.settings.rules.multiple !== false && i > 0) );
					});
			...
			select_node : function(obj,multiple){
				if(this.locked) return this.error("LOCKED");
				...
				try{
					...
					// NOTHING TO DO IT IS WHEN WE MASK THE NAVIGUATION AREA
				}
			},

			select_branch : function (obj, multiple) {
				var _this = this;
				this.select_node(obj,multiple);
				try{
	...
	tree_component.mousemove = function(event) {
		...
		if(tmp.isdown) {
			...
			if(cnt.size() == 0 || !tree_component.inst[cnt.attr("id")]) {
				...
				tmp.move_type	= false;
				tmp.ref_node	= false;
				//rree_component.drag_drop.marker.hide();
				return false;
			}
