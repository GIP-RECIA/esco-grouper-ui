...
var StemResume = new DUI.Class({
			...
			fluid.inlineEdit(".editableText", {
	            ...
	            listeners : {
	            	onFinishEdit : function(newValue, oldValue, editNode, viewNode) {
	            		...
	            		regIdNode = /^(.*):(\d+):(.*)$/gi;
	            		mymatch = regIdNode.exec(editNode.id);
	            		if (mymatch != null ){
		            		var json = {
		        				id : mymatch[2],
		        				newValue : newValue
		            		...
		            		$.post("/" + Core.applicationContext + "/ajax/stemController/getAttributeKey.jsf", json, function(data) {
		            			var res = Core.getResult(data);
								Core.log("Result of query : /ajax/stemController/getAttributeKey => " +  res);
								var key = res;

								$(editNode).addClass("validate[org.esco.grouperui.stem.attribut.regexp."+key+"]");
								editNode.name = "org.esco.grouperui.stem.attribut.regexp."+key;
								...
								if (!error) {
									$.post("/" + Core.applicationContext + "/ajax/stemController/applyModification.jsf", json, function(data) {
				        			});
								}
		        			});
	            		}
	...
	initAction : function() {
		...
		$(".errorLign").bind("click", function() {
			...
			$.post("/" + Core.applicationContext + "/ajax/stemController/discardModification.jsf", json, function(data) {
				_line.remove();
			});
		});
				...
				function(){
					_displayBlockUIOption = {
						onAfterShowBlockUI : function(){
							var json = {};
							$.post("/" + Core.applicationContext + "/ajax/stemController/saveStem.jsf", json, function(data) {
		...
		Core.addAction($("#groupCancel"),
				...
				function(){
					...
					var url = "/" + Core.applicationContext + "/stylesheets/stemModifications.jsf";
					var query= {stemUuid: $("input[id=stemUuid]").val()};
