...
;(function($) {
    ...
    $.sexyCombo = function(selectbox, config) {
	    ...
	    appendTo(this.wrapper).
	    addClass("icon").
		attr("id", "_"+this.selectbox.attr("name") + this.config.suffix)
		;

	    this.listWrapper = $("<div />").
	    appendTo($(this.config.listContainer)).
	    //addClass("invisible").
    ...
    $sc.fn.extend({
        ...
        initEvents: function() {
	        ...
	        this.icon.bind("click", function(e) {
					self.wrapper.data("sc:positionY", $("input[name="+e.target.id.substring(1,e.target.id.length)+"]").offset().top + 17);
					if ($.browser["msie"]){
						self.wrapper.data("sc:positionX", $("input[name="+e.target.id.substring(1,e.target.id.length)+"]").offset().left +1);
					}else{
						self.wrapper.data("sc:positionX", $("input[name="+e.target.id.substring(1,e.target.id.length)+"]").offset().left -2);
					}

	        });

	        this.input.bind("click", function(e) {
		            self.wrapper.data("sc:positionY", $("input[name="+e.target.name+"]").offset().top + 17);
		            if ($.browser["msie"]){
		            	self.wrapper.data("sc:positionX", $("input[name="+e.target.name+"]").offset().left +1);
		            }else{
		            	self.wrapper.data("sc:positionX", $("input[name="+e.target.name+"]").offset().left -2);
		            }
	        });
	    ...
	    showList: function() {
	        if (!this.listItems.filter(".visible").length)
	            return;
			//alert($(this.listWrapper).parent().attr("id"));
	        this.listWrapper.removeClass("invisible").
	        addClass("visible");

			$(this.listWrapper).css("top",parseInt(this.wrapper.data("sc:positionY")));
			$(this.listWrapper).css("left",parseInt(this.wrapper.data("sc:positionX")));
			$(this.listWrapper).parent().css("display","block");
			$(this.listWrapper).parent().css("zIndex","50");
	    ...
	    hideList: function() {
	        if (this.listWrapper.hasClass("invisible"))
	            return;
			$(this.listWrapper).parent().css("display","none");
	    ...
	    setOverflow: function() {
	        ...
	        if (this.getListItemsHeight() > maxHeight){
	            this.listWrapper.css("overflowY", "scroll");
				this.listWrapper.css("overflowX", "scroll");
			}
	        else{
	            this.listWrapper.css("overflowY", "hidden");
	...
	setListHeight: function() {
		...
		// this.listWrapper.css("maxHeight",this.getListMaxHeight());
